    ...
    **kwargs,
    ...
    if isinstance(dataset, dict):
        ...
        for t in range(len(dataset)):
            ctx.invoke(
                predict,
                output_folder=predictions_folder + str(t),
                model=model,
                task=t,
                dataset=dataset[t],
                batch_size=batch_size,
    ...
    if isinstance(dataset, dict):
        ...
        for t in range(len(dataset)):
            ctx.invoke(
                evaluate,
                output_folder=analysis_folder + str(t),
                predictions_folder=predictions_folder + str(t),
                dataset=dataset[t],
                second_annotator=second_annotator,
                overlayed=overlayed_folder,
                threshold=threshold,
    ...
    if isinstance(dataset, list):
        for t in range(len(dataset)):
            systems = []
            for k, v in dataset[t].items():
                if k.startswith("_"):
                    logger.info(f"Skipping dataset '{k}' (not to be compared)")
                    continue
                candidate = os.path.join(analysis_folder + str(t), f"{k}.csv")
                if not os.path.exists(candidate):
                    logger.error(
                        f"Skipping dataset '{k}' "
                        f"(candidate CSV file `{candidate}` does not exist!)"
                ...
                systems += [
                    k,
                    os.path.join(analysis_folder + str(t), f"{k}.csv"),
                ]
            if second_annotator is not None:
                for k, v in second_annotator.items():
                    if k.startswith("_"):
                        ...
                        continue
                    if k not in dataset[t]:
                        logger.info(
                            f"Skipping second-annotator '{k}' "
                            f"(no equivalent `dataset[{t}][{k}]`)"
                        )
                        continue
                    if not dataset[t][k].all_keys_match(v):
                        logger.warning(
                            f"Skipping second-annotator '{k}' "
                            f"(keys do not match `dataset[{t}][{k}]`?)"
                        )
                        continue
                    candidate = os.path.join(
                        analysis_folder, "second-annotator", f"{k}.csv"
            ...
            output_figure = os.path.join(
                output_folder, "comparison" + str(t) + ".pdf"
            )
            output_table = os.path.join(
                output_folder, "comparison" + str(t) + ".rst"
            )
