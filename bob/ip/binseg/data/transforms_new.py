...
class ResizeMasks:
    ...
    def __init__(self, reference='montgomery'):
        self.reference = reference
        ref_dict = dict({
            'montgomery': [0.7596635690567274, 0.649859773029811],
            'jsrt': [0.7973257211538461, 0.7816018787955465],
            'shenzhen': [0.7143327880448671, 0.6816013672614707],
            'cxr8': [0.7570281144086692, 0.7207913744760078]
        })
        self.ref_area = ref_dict[reference][0] * ref_dict[reference][1]


    def __call__(self, *args):

        img = numpy.array(args[1])

        h0, w0 = img.shape

        where = numpy.where(img)
        ...
        condition = (w * h) > (self.ref_area)

        limit = numpy.min([x0, w0 - x1, y0, h0 - y1])
        res = []
        ...
        def _pil_pad(img, x):
            new = PIL.Image.new(img.mode, (w0 + x, h0 + x), 0)
            new.paste(img, (x // 2, x // 2))
            return new

        if condition:
            for i in numpy.linspace(0.1, 1, 19):
                val = int(numpy.rint(limit * i))
                new_mask = _pil_pad(args[1], val)
                new_mask = numpy.array(new_mask)
                h_temp, w_temp = new_mask.shape
                where = numpy.where(img)
                wf = (numpy.max(where[1]) - numpy.min(where[1])) / w_temp
                hf = (numpy.max(where[0]) - numpy.min(where[0])) / h_temp
                res.append(abs((wf * hf) - self.ref_area))

            pad = int(numpy.rint(limit * numpy.linspace(0.1, 1, 19)[numpy.argmin(res)]))
            new_args = [_pil_pad(k, pad) for k in args]
            ...
            for i in numpy.linspace(0.1, 1, 19):
                ...
                val = int(numpy.rint(limit * i))
                new_mask = numpy.array(args[1].crop((val, val, w0 - val, h0 - val)))
                h_temp, w_temp = new_mask.shape
                where = numpy.where(img)
                wf = (numpy.max(where[1]) - numpy.min(where[1])) / w_temp
                hf = (numpy.max(where[0]) - numpy.min(where[0])) / h_temp
            ...
            crop = int(numpy.rint(limit * numpy.linspace(0.1, 1, 19)[numpy.argmin(res)]))
            new_args = [k.crop((crop, crop, w0 - crop, h0 - crop)) for k in args]
...
class CenterMasks:
    ...
    def __init__(self, reference='centered'):
        self.reference = reference
        ref_dict = dict({
            'montgomery': [0.4940054473623563, 0.41999774552056335],
            'jsrt': [0.5040169534412956, 0.4666446577682186],
            'shenzhen': [0.49629946077874504, 0.46466836858499794],
            'cxr8': [0.5001490799478238, 0.46549158058887796],
            'centered': [0.5, 0.5]
        })
        self.ref_xc, self.ref_yc = ref_dict[reference]

    def __call__(self, *args):

        def _pil_pad(img, x, y):
            abs_x = abs(x)
            abs_y = abs(y)
            if img.mode == 'P':
                img = img.convert('RGB')
            new = PIL.Image.new(img.mode, (w0 + abs_x, h0 + abs_y), 0)
            new.paste(img, ((abs_x // 2) - x, (abs_y // 2) - y))
            return new
        ...
        dx = (xc - (self.ref_xc * w0))
        dy = (yc - (self.ref_yc * h0))

        dx = numpy.rint(dx)
        dy = numpy.rint(dy)

        new_args = [_pil_pad(k, int(dx), int(dy)) for k in args]
