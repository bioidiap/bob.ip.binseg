...
def _load(data, threshold=None):
    ...
    threshold : :py:class:`float`, :py:class:`str`, Optional
        A value indicating which threshold to choose for selecting a "IoU-score"
        If set to ``None``, then use the maximum IoU-score on that measures file.
        If set to a floating-point value, then use the IoU-score that is
        ...
        threshold reaching the maximum IoU-score on that particular dataset and
