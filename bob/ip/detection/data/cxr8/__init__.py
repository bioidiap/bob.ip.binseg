...
def _raw_data_loader(sample):
    retval = dict(
        ...
        label=numpy.array(
            Image.open(os.path.join(_root_path, sample["label"]))
        ),
    )

    retval["label"] = numpy.where(retval["label"] == 255, 0, retval["label"])
    retval["label"] = Image.fromarray(numpy.array(retval["label"] > 0))
    return retval
