...
\b
  1. Runs prediction on an existing dataset configuration:
     ...
     .. code:: sh
        ...
        $ binseg predict -vv m2unet drive --weight=path/to/model_final_epoch.pth --output-folder=path/to/predictions
...
\b
  2. To run prediction on a folder with your own images, you must first
     ...
     .. code:: sh
        ...
        $ binseg predict -vv m2unet mydataset.py --weight=path/to/model_final_epoch.pth --output-folder=path/to/predictions
""",
)
@click.option(
    "--output-folder",
